...
//==========public react============//
// Auto React - ONLY 2 EMOJIS
if (!isReact && senderNumber !== botNumber) {
    if (config.AUTO_REACT === 'true') {
        const reactions = ['❤️', '🔥']; // Only 2 emojis
        const randomReaction = reactions[Math.floor(Math.random() * reactions.length)];
        m.react(randomReaction);
    }
}

// Owner React - ONLY 2 EMOJIS
if (!isReact && senderNumber === botNumber) {
    if (config.AUTO_REACT === 'true') {
        const reactions = ['👑', '⭐']; // Only 2 emojis
...
if (!isReact && senderNumber !== botNumber) {
    if (config.CUSTOM_REACT === 'true') {
        const reactions = (config.CUSTOM_REACT_EMOJIS || '🥲,😂').split(','); // Only 2 default
        const randomReaction = reactions[Math.floor(Math.random() * reactions.length)];
        m.react(randomReaction);
    }
}
...
// FANA COMMAND HANDLER
const { commands } = require('./njabulo');

if (isCmd && command) {
    const fanaCommand = commands.find(c => c.name === command.toLowerCase());
    
    if (fanaCommand) {
        if (fanaCommand.fromMe && !isOwner) {
            return reply('❌ This command is for owner only!');
        }
        
        try {
            const result = await fanaCommand.function(conn, mek, args, {
                conn, mek, args, body, command, q, text,
                from, isGroup, sender, senderNumber, botNumber,
                pushname, isOwner, reply, prefix, m,
            ...
            console.error('Fana command error:', error);
