...
cmd({
  pattern: "invert",
  alias: ["invertedit", "negative", "invertcolor"],
  react: '📸',
  ...
  try {
    ...
    const tempFilePath = path.join(os.tmpdir(), `img_invert_${Date.now()}${extension}`);
    ...
    await conn.sendMessage(from, {
      image: imageBuffer,
      caption: `📸 *Invert Effect Applied!*\n\n✅ Image colors inverted successfully!`
    }, { quoted: mek });

    await sendFormattedMessage(
      conn, 
      from, 
      `✅ *Image inverted successfully!*\n\n📸 Color inversion applied to your image.\n\nEnjoy your edited image!`, 
      sender, 
      pushname,
      "Invert Colors - Success",
      "Image delivered"
    );
