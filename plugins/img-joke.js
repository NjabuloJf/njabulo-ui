...
cmd({
  pattern: "imgjoke",
  alias: ["jokedit", "jokeoverhead", "funny"],
  react: '📸',
  desc: "Add joke/overhead text to images",
  category: "img_edit",
  ...
  try {
    ...
    if (!mimeType || !mimeType.startsWith('image/')) {
      await sendFormattedMessage(
        conn, 
        from, 
        "📸 *Please reply to an image file*\n\n📌 *Usage:* Reply to an image with .imgjoke\n📝 *Supported formats:* JPEG, PNG", 
        sender, 
        pushname,
        "Joke Effect - Error",
        "No image"
    ...
    const tempFilePath = path.join(os.tmpdir(), `img_joke_${Date.now()}${extension}`);
    ...
    // Apply joke effect using the API
    ...
    await conn.sendMessage(from, {
      image: imageBuffer,
      caption: `📸 *Joke Effect Applied!*\n\n😂 Image processed with joke overhead text!`
    }, { quoted: mek });

    await sendFormattedMessage(
      conn, 
      from, 
      `✅ *Joke effect applied successfully!*\n\n📸 Your image now has a funny joke overlay.\n\nEnjoy your edited image!`, 
      sender, 
      pushname,
      "Joke Effect - Success",
      "Image delivered"
