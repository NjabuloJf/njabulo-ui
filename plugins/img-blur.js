...
cmd({
  pattern: "blur",
  alias: ["bluredit", "blurimage", "imageblur"],
  react: '📸',
  desc: "Blur an image",
  category: "img_edit",
  ...
  try {
    ...
    const tempFilePath = path.join(os.tmpdir(), `imgblur_${Date.now()}${extension}`);
    ...
    await sendFormattedMessage(
      conn, 
      from, 
      `✅ *Image processed successfully!*\n\n📸 Blur effect applied to your image.\n\nEnjoy your edited image!`, 
      sender, 
      pushname,
      "Blur Image - Success",
      "Image delivered"
